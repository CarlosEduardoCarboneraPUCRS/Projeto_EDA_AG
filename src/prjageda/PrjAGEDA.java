...
public class PrjAGEDA {
    ...
    public static void main(String[] args) throws Exception, Throwable {
         ...
         *     - Retornar a Árvore com todas as suas Sub-Árvores geradas
         ...
         * 
         * OBSERVAÇÃO: 1°) A Opção DEEPCOPY não funciona para objetos do tipo "ArrayList<>", ou seja, apartir da posição 1 o mesmo guardará as referências do Objetos vinculado
         *             2°) Cuidar com o nível de profundidade da árvore, quando do "CROSSOVER E DA MUTAÇÃO", poderá ocorrer caso em que o nível de profundidade da árvore DUPLIQUE.
         *             
         * >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  ATENÇÃO <<<<<<<<<<<<<<<<<<<<<<  ATENÇÃO <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  ATENÇÃO <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
         -------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
